...
module GameFunctions
  ...
  def comp_guess(colors)
    ...
    loop do
      ...
      break if player_feedback.all?('correct') || player_feedback.all?('c') || rounds == 12
      ...
      player_feedback.each_with_index do |f, i|
        if f == 'incorrectplace' || f == 'ic'
          loop do
            random_index = rand(4)
            # binding.pry
            if player_feedback[random_index] == 'correct' || player_feedback[random_index] == 'c' || random_index == i
              next
            else
  ...
  def get_player_feedback
    ...
    loop do
      ...
      if correct_response_indicator == 4
        ...
        puts "Please only enter specified feedback responses (Wrong, Correct, Incorrect place OR W, C, IC)"
        next
      end
    end
  end
